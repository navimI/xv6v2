...
sys_dup2(void)
{
  ...
  //comprobaciones:
  // 1. comprobar que el arg 0 es un fich que esta abierto
  //    argfd
  if(argfd(0, &oldfd, &oldf) < 0)//si el fichero no esta abierto da error
    return -1;

  // 2. comprobar que el arg 1 es un numero valido dentro del rango
  //    argint y luego comprobar que el fd esta dentro de los rangos 0 y NOFILE
  if(argint(1, &newfd) < 0)
    return -1;
  ...
  // 3. si arg 0 == arg 1 return oldfd
  if(oldfd == newfd)
    return oldfd;

  //implementacion:
  // 4. si newfd esta abierto lo cierras
  //    ofile[newfd] != 0 ==> fileclose(fd)
  if((newf = curproc->ofile[newfd])!= 0)
    fileclose(newf);
